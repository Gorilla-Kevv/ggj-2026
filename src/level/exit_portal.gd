...
@export var portal_color: Color = Color(0.3, 0.9, 0.6)   # 传送门颜色
...
func _draw() -> void:
	# 发光环 (外圈到内圈)
	for i in range(3):
		draw_arc(Vector2.ZERO, 40.0 + i * 7.0, 0, TAU, 32, Color(portal_color, 0.35 - i * 0.1), 3.0)
	# 主环
	draw_arc(Vector2.ZERO, 40.0, 0, TAU, 32, portal_color, 4.0)
